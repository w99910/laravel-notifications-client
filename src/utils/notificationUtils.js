 ...
 */
export const formatTime = (isoString, currentTime, locales = {}) => {
  if (!isoString) return "";
  ...
  if (minutes < 1) return locales["Just now"] ?? "Just now";
  if (minutes < 60)
    return `${minutes} ${locales["minute"] ?? "minute"}${
      minutes > 1 ? "s" : ""
    } ${locales["ago"] ?? "ago"}`;
  if (hours < 24)
    return `${hours} ${locales["hour"] ?? "hour"}${hours > 1 ? "s" : ""} ${
      locales["ago"] ?? "ago"
    }`;
  if (days < 7)
    return `${days} ${locales["day"] ?? "day"}${days > 1 ? "s" : ""} ${
      locales["ago"] ?? "ago"
    }`;
  if (weeks < 4)
    return `${weeks} ${locales["week"] ?? "week"}${weeks > 1 ? "s" : ""} ${
      locales["ago"] ?? "ago"
    }`;
  if (months < 12)
    return `${months} ${locales["month"] ?? "month"}${months > 1 ? "s" : ""} ${
      locales["ago"] ?? "ago"
    }`;
  return `${years} ${locales["year"] ?? "year"}${years > 1 ? "s" : ""} ${
    locales["ago"] ?? "ago"
  }`;
};
